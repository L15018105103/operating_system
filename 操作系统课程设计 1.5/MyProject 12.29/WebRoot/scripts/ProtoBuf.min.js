...
(function (s) {
    function u(n) {
        ...
        f.TYPES = {
            int32: {name: "int32", wireType: f.WIRE_TYPES.VARINT},
            uint32: {name: "uint32", wireType: f.WIRE_TYPES.VARINT},
            sint32: {name: "sint32", wireType: f.WIRE_TYPES.VARINT},
            int64: {name: "int64", wireType: f.WIRE_TYPES.VARINT},
            uint64: {name: "uint64", wireType: f.WIRE_TYPES.VARINT},
            sint64: {name: "sint64", wireType: f.WIRE_TYPES.VARINT},
            bool: {name: "bool", wireType: f.WIRE_TYPES.VARINT},
            "double": {name: "double", wireType: f.WIRE_TYPES.BITS64},
            string: {name: "string", wireType: f.WIRE_TYPES.LDELIM},
            bytes: {name: "bytes", wireType: f.WIRE_TYPES.LDELIM},
            fixed32: {name: "fixed32", wireType: f.WIRE_TYPES.BITS32},
            sfixed32: {name: "sfixed32", wireType: f.WIRE_TYPES.BITS32},
            fixed64: {name: "fixed64", wireType: f.WIRE_TYPES.BITS64},
            sfixed64: {name: "sfixed64", wireType: f.WIRE_TYPES.BITS64},
            "float": {name: "float", wireType: f.WIRE_TYPES.BITS32},
            "enum": {name: "enum", wireType: f.WIRE_TYPES.VARINT},
            message: {name: "message", wireType: f.WIRE_TYPES.LDELIM},
            group: {name: "group", wireType: f.WIRE_TYPES.STARTGROUP}
        };
        f.ID_MIN = 1;
        f.ID_MAX = 536870911;
        f.ByteBuffer = n;
        ...
        f.Util = function () {
            ...
            c.fetch = function (e, f) {
                ...
                if (c.IS_NODE) if (f) require("fs").readFile(e, function (d, a) {
                    d ? f(null) : f("" + a)
                }); else try {
        ...
        f.DotProto = function (c, e) {
            ...
            q._readString = function () {
                ...
                var d;
                if (null !== (d = e.STRING.exec(this.source))) return d =
            ...
            q.next = function () {
                ...
                d = this.index;
                e.DELIM.lastIndex = 0;
                if (e.DELIM.test(this.source.charAt(d))) ++d; else for (++d; d < this.source.length && !e.DELIM.test(this.source.charAt(d));) d++;
            ...
            b._parseNumber = function (d) {
                ...
                if (e.NUMBER_FLT.test(d)) return "inf" === d ? Infinity * a : "nan" === d ? NaN : a * parseFloat(d);
            ...
            b._parseId = function (d, a) {
                ...
                if (e.NUMBER_DEC.test(d)) h = parseInt(d); else if (e.NUMBER_HEX.test(d)) h = parseInt(d.substring(2), 16); else if (e.NUMBER_OCT.test(d)) h = parseInt(d.substring(1), 8); else throw Error("Illegal id at line " + this.tn.line + ": " + (0 > b ? "-" : "") + d);
                h = b * h | 0;
            ...
            b._parseOption = function (d, a) {
                ...
                if (a !== e.EQUAL) throw Error("Illegal operator in message " + d.name + ", option " + b + " at line " + this.tn.line +
                    ": " + a);
                a = this.tn.peek();
                if (a === e.STRINGOPEN || a === e.STRINGOPEN_SQ) h = this._parseString(); else if (this.tn.next(), e.NUMBER.test(a)) h = this._parseNumber(a, !0); else if (e.BOOL.test(a)) h = "true" === a; else if (e.TYPEREF.test(a)) h = a; else throw Error("Illegal option value in message " + d.name + ", option " + b + " at line " + this.tn.line + ": " + a);
                a = this.tn.next();
            ...
            b._parseService = function (d, a) {
                ...
                do if (a = this.tn.next(), "option" === a) this._parseOption(b, a); else if ("rpc" === a) this._parseServiceRPC(b,
                    a); else if (a !== e.CLOSE) throw Error("Illegal type of service " + h + " at line " + this.tn.line + ": " + a); while (a !== e.CLOSE);
                d.services.push(b)
            };
            b._parseServiceRPC = function (d, a) {
                ...
                a = this.tn.next();
                if (a !== e.COPTOPEN) throw Error("Illegal start of request type in service " + d.name + "#" + c + " at line " +
                    this.tn.line + ": " + a);
                a = this.tn.next();
                ...
                a = this.tn.next();
                if ("returns" !== a.toLowerCase()) throw Error("Illegal delimiter in service " + d.name + "#" + c + " at line " + this.tn.line + ": " +
                    a);
                a = this.tn.next();
                if (a != e.COPTOPEN) throw Error("Illegal start of response type in service " + d.name + "#" + c + " at line " + this.tn.line + ": " + a);
                a = this.tn.next();
                ...
                a = this.tn.next();
                if (a === e.OPEN) {
                    do if (a = this.tn.next(), "option" === a) this._parseOption(f, a); else if (a !== e.CLOSE) throw Error("Illegal start of option inservice " +
                        d.name + "#" + c + " at line " + this.tn.line + ": " + a); while (a !== e.CLOSE);
                    this.tn.peek() === e.END && this.tn.next()
                } else if (a !== e.END) throw Error("Illegal delimiter in service " + d.name + "#" + c + " at line " + this.tn.line + ": " + a);
            ...
            b._parseMessage = function (d, a, b) {
                ...
                if (f) {
                    b = this.tn.next();
                    if (b !==
                        e.EQUAL) throw Error("Illegal id assignment after group " + c.name + " at line " + this.tn.line + ": " + b);
                    b = this.tn.next();
                    try {
            ...
            b._parseMessageField = function (b, a) {
                ...
                if ("group" === a) {
                    ...
                    c.name = a;
                    a = this.tn.next();
                    if (a !== e.EQUAL) throw Error("Illegal token in field " + b.name + "#" + c.name + " at line " + this.tn.line + ": " + a);
                    a = this.tn.next();
                    try {
                    ...
                    if (a !== e.END) throw Error("Illegal delimiter in message " +
            ...
            b._parseMessageOneOf = function (b, a) {
                a = this.tn.next();
                if (!e.NAME.test(a)) throw Error("Illegal oneof name in message " + b.name + " at line " + this.tn.line + ": " + a);
                var c = a, f, k = [];
                a = this.tn.next();
            ...
            b._parseFieldOptions = function (b,
                                             a, c) {
                var f = !0;
                do {
                    c = this.tn.next();
                    if (c === e.OPTCLOSE) break; else if (c === e.OPTEND) {
            ...
            b._parseFieldOption = function (b, a, c) {
                ...
                c = this.tn.next();
                if (f) {
                    if (c !== e.COPTCLOSE) throw Error("Illegal delimiter in " +
                        b.name + "#" + a.name + " at line " + this.tn.line + ": " + c);
                    k = "(" + k + ")";
                    c = this.tn.next();
                    e.FQTYPEREF.test(c) && (k += c, c = this.tn.next())
                }
                if (c !== e.EQUAL) throw Error("Illegal token in " + b.name + "#" + a.name + " at line " + this.tn.line + ": " + c);
                c = this.tn.peek();
                if (c === e.STRINGOPEN || c === e.STRINGOPEN_SQ) b = this._parseString(); else if (e.NUMBER.test(c, !0)) b = this._parseNumber(this.tn.next(), !0); else if (e.BOOL.test(c)) b = "true" === this.tn.next().toLowerCase(); else if (e.TYPEREF.test(c)) b = this.tn.next(); else throw Error("Illegal value in message " +
            ...
            b._parseEnum = function (b, a) {
                var c = {};
                a = this.tn.next();
                if (!e.NAME.test(a)) throw Error("Illegal enum name in message " + b.name + " at line " + this.tn.line + ": " + a);
                c.name = a;
                a = this.tn.next();
            ...
            b._parseExtend = function (b, a) {
                a = this.tn.next();
                if (!e.TYPEREF.test(a)) throw Error("Illegal message name at line " + this.tn.line + ": " + a);
                var c = {};
                c.ref = a;
                c.fields = [];
                a = this.tn.next();
                if (a !== e.OPEN) throw Error("Illegal start of extend " + c.name + " at line " + this.tn.line + ": " + a);
                do if (a = this.tn.next(), a === e.CLOSE) {
                    a = this.tn.peek();
                    a == e.END && this.tn.next();
                    break
                } else if (e.RULE.test(a)) this._parseMessageField(c,
                    a); else throw Error("Illegal token in extend " + c.name + " at line " + this.tn.line + ": " + a); while (1);
                b.messages.push(c);
                return c
            };
            b.toString = function () {
        ...
        f.Reflect = function (c) {
            ...
            function f(a, b) {
                ...
                if ("number" === typeof a) return c.Long.fromNumber(a,
                    b || !1);
                throw Error("not convertible to Long");
            }

            var l = {}, q = function (a, b, c) {
                this.builder = a;
                this.parent = b;
                this.name = c
            }, b = q.prototype;
            ...
            b.addChild = function (b) {
                var c;
                if (c = this.getChild(b.name)) if (c instanceof a.Field && c.name !== c.originalName && null === this.getChild(c.originalName)) c.name = c.originalName; else if (b instanceof a.Field && b.name !== b.originalName && null === this.getChild(b.originalName)) b.name = b.originalName; else throw Error("Duplicate name in namespace " +
            ...
            b.resolve = function (a, b) {
                ...
                do {
                    ...
                    if (null != d) break;
                    if (null !== this.parent) return this.parent.resolve(a,
                        b)
                } while (null != d);
                return d
            };
            ...
            h.build = function (b) {
                ...
                b = function (a, b) {
                    ...
                    var d = b.getChildren(a.Reflect.Message.Field), m = b.getChildren(a.Reflect.Message.OneOf),
                        e = function (b, c) {
                            ...
                            if (0 < arguments.length) if (1 !==
                                arguments.length || "object" !== typeof b || "function" === typeof b.encode || a.Util.isArray(b) || b instanceof n || b instanceof ArrayBuffer || a.Long && b instanceof a.Long) for (e = 0, p = arguments.length; e < p; ++e) "undefined" !== typeof(f = arguments[e]) && this.$set(d[e].name, f); else this.$set(b)
                    ...
                    p.add = function (c, d, m) {
                        ...
                        if (!m) {
                            ...
                            if (!(e instanceof a.Reflect.Message.Field)) throw Error(this + "#" + c + " is not a field: " +
                                e.toString(!0));
                ...
                this._fieldsById = {};
                this._fieldsByName = {};
                for (var d = 0, e = this.children.length, f; d < e; d++) if (f = this.children[d], f instanceof k) b[f.name] = f.build(); else if (f instanceof a) b[f.name] = f.build(); else if (f instanceof a.Field) f.build(), this._fields.push(f), this._fieldsById[f.id] = f, this._fieldsByName[f.name] = f; else if (!(f instanceof a.OneOf || f instanceof g)) throw Error("Illegal reflect child of " + this.toString(!0) + ": " + children[d].toString(!0));
                return this.clazz = b
            };
            h.encode = function (a, b, c) {
                for (var d = null,
                         e, f = 0, h = this._fields.length, g; f < h; ++f) e = this._fields[f], g = a[e.name], e.required && null === g ? null === d && (d = e) : e.encode(c ? g : e.verifyValue(g), b);
            ...
            h.decode =
                function (a, b, d) {
                    ...
                    for (var f = a.offset, h = new this.clazz, g, k, l; a.offset < f + b || -1 === b && 0 < a.remaining();) {
                        ...
                        if (g = this._fieldsById[l]) g.repeated && !g.options.packed ? h[g.name].push(g.decode(k, a)) : (h[g.name] = g.decode(k, a), g.oneof && (null !== this[g.oneof.name] && (this[this[g.oneof.name]] = null), h[g.oneof.name] =
                    ...
                    a = 0;
                    for (b = this._fields.length; a < b; ++a) if (g = this._fields[a], null === h[g.name]) {
                        if (g.required) throw a = Error("Missing at least one required field for " +
            ...
            l.Message = a;
            var r = function (b, c, d, e, f, h, g, k) {
                q.call(this, b, c, f);
            ...
            h.verifyValue = function (a, b) {
                ...
                switch (this.type) {
                    ...
                    case c.TYPES.bytes:
                        return n.isByteBuffer(a) ?
                            a : n.wrap(a, "base64");
                    case c.TYPES["enum"]:
                        var l = this.resolvedType.getChildren(k.Value);
                        for (e = 0; e < l.length; e++) if (l[e].name == a || l[e].id == a) return l[e].id;
            ...
            h.encode = function (a, b) {
                ...
                try {
                    if (this.repeated) {
                        ...
                        } else for (d = 0; d < a.length; d++) b.writeVarint32(this.id << 3 | this.type.wireType), this.encodeValue(a[d], b)
            ...
            h.calculateValue = function (a) {
                ...
                switch (this.type) {
                    ...
                    case c.TYPES.group:
                        return a = this.resolvedType.calculate(a), a + n.calculateVarint32(this.id <<
            ...
            (k.prototype = Object.create(d.prototype)).build = function () {
                for (var a =
                    {}, b = this.getChildren(k.Value), c = 0, d = b.length; c < d; ++c) a[b[c].name] = b[c].id;
                Object.defineProperty && Object.defineProperty(a, "$options", {value: this.buildOpt()});
                ...
                return this.clazz && !a ? this.clazz : this.clazz = function (a, b) {
                    for (var c = function (b) {
                        ...
                        Object.defineProperty && (Object.defineProperty(c[a.name], "$options",
                            {value: a.buildOpt()}), Object.defineProperty(d[a.name], "$options", {value: c[a.name].$options}))
                    })(e[f]);
            ...
            (t.prototype = Object.create(q.prototype)).buildOpt =
                b.buildOpt;
            l.Service.Method = t;
            b = function (a, b, c, d, e, f, h, g) {
                t.call(this, a, b, c, g);
                this.className = "Service.RPCMethod";
                this.requestName = d;
                this.responseName = e;
                this.requestStream = f;
                this.responseStream = h;
        ...
        f.Builder = function (c, e, f) {
            ...
            q.define = function (b) {
                ...
                b = b.split(".");
                var c, a;
                for (c = 0; c < b.length; c++) if (!e.NAME.test(b[c])) throw Error("Illegal package: " + b[c]);
                for (c = 0; c < b.length; c++) a = this.ptr.getChild(b[c]), null === a && this.ptr.addChild(a = new f.Namespace(this, this.ptr, b[c])), this.ptr = a;
                return this
            };
            l.isValidMessage = function (b) {
                if ("string" !== typeof b.name || !e.NAME.test(b.name) || "undefined" !== typeof b.values ||
                    "undefined" !== typeof b.rpc) return !1;
                var d;
                ...
                if ("undefined" !== typeof b.enums) {
                    ...
                    for (d = 0; d < b.enums.length; d++) if (!l.isValidEnum(b.enums[d])) return !1
                }
                if ("undefined" !== typeof b.messages) {
                    if (!c.Util.isArray(b.messages)) return !1;
                    for (d = 0; d < b.messages.length; d++) if (!l.isValidMessage(b.messages[d]) &&
                        !l.isValidExtend(b.messages[d])) return !1
                }
            ...
            l.isValidMessageField = function (b) {
                if ("string" !== typeof b.rule || "string" !== typeof b.name || "string" !== typeof b.type || "undefined" === typeof b.id || !(e.RULE.test(b.rule) && e.NAME.test(b.name) && e.TYPEREF.test(b.type) && e.ID.test("" + b.id))) return !1;
                if ("undefined" !== typeof b.options) {
                    if ("object" !==
                        typeof b.options) return !1;
                    for (var c = Object.keys(b.options), a = 0, f; a < c.length; a++) if ("string" !== typeof(f = c[a]) || "string" !== typeof b.options[f] && "number" !== typeof b.options[f] && "boolean" !== typeof b.options[f]) return !1
                }
                return !0
            };
            l.isValidEnum = function (b) {
                ...
                for (var d = 0; d < b.values.length; d++) if ("object" != typeof b.values[d] || "string" !== typeof b.values[d].name || "undefined" ===
                    typeof b.values[d].id || !e.NAME.test(b.values[d].name) || !e.NEGID.test("" + b.values[d].id)) return !1;
                return !0
            };
            q.create = function (b) {
                if (!b) return this;
                ...
                for (d.push(b); 0 < d.length;) {
                    b = d.pop();
                    if (c.Util.isArray(b)) for (; 0 < b.length;) {
                        var a = b.shift();
                        if (l.isValidMessage(a)) {
                            var e = new f.Message(this, this.ptr, a.name, a.options, a.isGroup), q = {};
                            if (a.oneofs) for (var k = Object.keys(a.oneofs), g = 0, n = k.length; g < n; ++g) e.addChild(q[k[g]] = new f.Message.OneOf(this,
                                e, k[g]));
                            if (a.fields && 0 < a.fields.length) for (g = 0, n = a.fields.length; g < n; ++g) {
                                k = a.fields[g];
                                if (null !== e.getChild(k.id)) throw Error("Duplicate field id in message " + e.name + ": " + k.id);
                                if (k.options) for (var m = Object.keys(k.options), p = 0, s = m.length; p < s; ++p) {
                                    if ("string" !== typeof m[p]) throw Error("Illegal field option name in message " + e.name + "#" + k.name + ": " + m[p]);
                                    if ("string" !== typeof k.options[m[p]] && "number" !== typeof k.options[m[p]] && "boolean" !== typeof k.options[m[p]]) throw Error("Illegal field option value in message " +
                            ...
                            q = [];
                            if ("undefined" !== typeof a.enums && 0 < a.enums.length) for (g = 0; g < a.enums.length; g++) q.push(a.enums[g]);
                            if (a.messages && 0 < a.messages.length) for (g = 0; g < a.messages.length; g++) q.push(a.messages[g]);
                            a.extensions &&
                            ...
                            for (g in a.rpc) a.rpc.hasOwnProperty(g) &&
                            ...
                            if (a.fields[g].id < e.extensions[0] || a.fields[g].id > e.extensions[1]) throw Error("Illegal extended field id in message " + e.name + ": " +
            ...
            q["import"] = function (b, d) {
                ...
                if (b.imports && 0 < b.imports.length) {
                    var e = "/", f = !1;
                    if ("object" === typeof d) {
                        if (this.importRoot = d.root, f = !0, a = this.importRoot, d = d.file, 0 <= a.indexOf("\\") || 0 <= d.indexOf("\\")) e = "\\"
                    ...
                    for (var k = 0; k < b.imports.length; k++) if ("string" === typeof b.imports[k]) {
                        ...
                        if (!/^google\/protobuf\//.test(g) && (g = a + e + g, !0 !== this.files[g])) {
                            ...
                            if (/\.json$/i.test(g)) this["import"](JSON.parse(l + ""), g); else this["import"]((new c.DotProto.Parser(l + "")).parse(), g)
                        }
            ...
            q.resolveAll = function () {
                ...
                if (null != this.ptr && "object" !== typeof this.ptr.type) {
                    if (this.ptr instanceof f.Namespace) {
                    ...
                    } else if (this.ptr instanceof f.Message.Field) if (e.TYPE.test(this.ptr.type)) this.ptr.type = c.TYPES[this.ptr.type]; else {
                        ...
                        if (b instanceof f.Enum) this.ptr.type = c.TYPES["enum"]; else if (b instanceof f.Message) this.ptr.type = b.isGroup ? c.TYPES.group : c.TYPES.message; else throw Error("Illegal type reference in " + this.ptr.toString(!0) + ": " + this.ptr.type);
                        ...
                        if (!(b && b instanceof c.Reflect.Message)) throw Error("Illegal type reference in " +
                    ...
                    } else throw Error("Illegal service type in " + this.ptr.toString(!0)); else if (!(this.ptr instanceof c.Reflect.Message.OneOf || this.ptr instanceof c.Reflect.Extension)) throw Error("Illegal object in namespace: " + typeof this.ptr +
                        ":" + this.ptr);
                    this.reset()
                }
            };
            q.build = function (b) {
                ...
                if (b) {
                    ...
                    for (var c = this.result, a = 0; a < b.length; a++) if (c[b[a]]) c = c[b[a]]; else {
                        c = null;
                        break
                    }
                    return c
        ...
        f.loadProto = function (c, e, n) {
            ...
            return f.loadJson((new f.DotProto.Parser(c)).parse(), e, n)
        };
        ...
        f.loadProtoFile = function (c, e, n) {
            ...
            if (e) return f.Util.fetch("string" === typeof c ? c : c.root + "/" + c.file, function (l) {
                if (null === l) e(Error("Failed to fetch file")); else try {
        ...
        f.newBuilder = function (c) {
            c = c || {};
            "undefined" === typeof c.convertFieldsToCamelCase && (c.convertFieldsToCamelCase = f.convertFieldsToCamelCase);
